...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 只需要给定输出特征图的大小就好，其中通道数前后不发生变化
...
# -----------------------------------------------------------------------------------------------------------------------------
'''
基于归一化的注意模块（NAM），它可以抑制不太显著的权重。它对注意模块应用了权重稀疏性惩罚，从而使它们在保持相似性能的同时，计算效率更高
'''
...
# ----------------------------------------------------------------------------------------------------------------------
'''
ECANet主要对SENet模块进行了一些改进，提出了一种不降维的局部跨信道交互策略（ECA模块）和自适应选择一维卷积核大小的方法
'''
class ECA(nn.Module):
    '''
    Args:
